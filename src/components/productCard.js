        ...
        <>
            <div className="card-thumbnail m-1 border border-2 border-dark" onMouseOver={() => setState(" d-flex animate__fadeIn")} onMouseLeave={() => setState(" animate__fadeOut d-flex")}>
                ...
                <div className="px-2 pb-2 d-flex align-items-baseline bg-light">
                    <Link to={`/products/${product.product.id}`} className="btn btn-sm c-coffee text-white mt-1 flex-fill mr-5">Ver más</Link>
                    <div className="d-flex justify-content-center mt-3 btn-group w-50 py-3" role="group">
                        <input id="cartQuantity" type="number" name="productNumber" className="btn btn-sm border-accent w-50" defaultValue="1" onChange={actions.handleChangeLogin} />
                        <button className="btn btn-sm c-accent w-75" onClick={() => actions.cartProducts(product.product, parseInt(store.productNumber))}><i className="fas fa-cart-plus"></i></button>
                    </div>
                </div>
            </div>
        </>
